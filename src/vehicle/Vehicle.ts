...
import { SetConfigParams, TrackMaterial } from "../message/c2v/SetConfigParams";
...
class Vehicle implements IVehicle {
	...
	public changeLane(offset: number, speed = 300, acceleration = 300, hopIntent = 0x0, tag = 0x0): void {
		this.writeAndPublish(new ChangeLane(this.id, offset, speed, acceleration, hopIntent, tag));
	}
	...
	public connect(): Promise<Vehicle> {
		...
		return new Promise<Vehicle>((resolve, reject) => {
			self._device
				.connect(ANKI_STR_CHR_READ_UUID, ANKI_CHR_WRITE_UUID)
				.then(() => {
					self.enableSdkMode();
	...
	public queryBatterLevel(): Promise<number> {
		...
		return new Promise<number>((resolve, reject) => {
			self.sendRequest<BatteryLevelRequest, BatteryLevelResponse>(new BatteryLevelRequest(this.id), ANKI_VEHICLE_MSG_V2C_BATTERY_LEVEL_RESPONSE)
	...
	public queryPing(): Promise<number> {
		...
		return new Promise<number>((resolve, reject) => {
			...
			self.sendRequest<PingRequest, PingResponse>(request, ANKI_VEHICLE_MSG_V2C_PING_RESPONSE)
	...
	public queryVersion(): Promise<number> {
		...
		return new Promise<number>((resolve, reject) => {
			self.sendRequest<VersionRequest, VersionResponse>(new VersionRequest(this.id), ANKI_VEHICLE_MSG_V2C_VERSION_RESPONSE)
				.then((message: VersionResponse) => resolve(message.version))
				.catch(reject);
		});
	}

	public setConfigParams(superCodeParseMask: number, trackMaterial: TrackMaterial): void {
		this.writeAndPublish(new SetConfigParams(this.id, superCodeParseMask, trackMaterial));
	}

	public setOffset(offset: number): void {
		this.writeAndPublish(new SetOffsetFromRoadCenter(this.id, offset));
		this._offset = offset;
	}

	public setSpeed(speed: number, acceleration = 500, limit = false): void {
		this.writeAndPublish(new SetSpeed(this.id, speed, acceleration, limit));
	}

	public setLights(lightsMask: number): void {
		this.writeAndPublish(new SetLights(this.id, lightsMask));
	}

	public setLightPattern(channels: LightChannelConfig[]): void {
		this.writeAndPublish(new SetLightPattern(this.id, channels));
	}

	public turnLeft(): void {
		this.writeAndPublish(new Turn(this.id, TurnType.VEHICLE_TURN_LEFT));
	}

	public turnRight(): void {
		this.writeAndPublish(new Turn(this.id, TurnType.VEHICLE_TURN_RIGHT));
	}

	public uTurn(): void {
		this.writeAndPublish(new Turn(this.id, TurnType.VEHICLE_TURN_UTURN));
	}

	public uTurnJump(): void {
		this.writeAndPublish(new Turn(this.id, TurnType.VEHICLE_TURN_UTURN_JUMP));
	}
	...
	private readAndPublish(payload: Buffer): void {
		...
		this._listeners.forEach(listener => listener(self._builder.messageId(payload.readUInt8(1)).vehicleId(self.id).payload(payload).build()));
	}
	...
	private sendRequest<Req extends IVehicleMessage, Res extends IVehicleMessage>(request: Req, responseId: number): Promise<Res> {
		const self = this;
